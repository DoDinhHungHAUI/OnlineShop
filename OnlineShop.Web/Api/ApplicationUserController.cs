...
namespace OnlineShop.Web.Api
{
    ...
    public class ApplicationUserController : ApiControllerBase
    {
        ...
        public HttpResponseMessage GetListUser(HttpRequestMessage request)
        {
        ...
        [HttpGet]
        public HttpResponseMessage GetListGroup(HttpRequestMessage request , string userId)
        {
            if (string.IsNullOrEmpty(userId))
            {
                return request.CreateErrorResponse(HttpStatusCode.BadRequest, nameof(userId) + " Không có giá trị.");
            }
            var listGroup = _appGroupService.GetListGroupByUserId(userId);
